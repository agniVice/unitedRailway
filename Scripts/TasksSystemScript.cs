...
public class TasksSystemScript : MonoBehaviour
{
    ...
    public void CheckDoneTasks()
    {
        ...
        uiScript.tasksGameOver.text = t + "/" + o;
        uiScript.tasksPauseText.text = t + "/" + o;
    }
        ...
        for (int i = 0; i < task.Count; i++)
        {
            if (isRepair == false)
            {
                if (isSell == false)
                {
                    ...
                    if (task[i].type == name + "Train" + tScript.placeWhereIAm.townName)
                    {
                        if (!task[i].isDone())
                        {
                            task[i].TakeInfo(count);
    ...
    public void GetInfo(RailRoadSystemScript rScript)
    {
        for (int i = 0; i < task.Count; i++)
        {
            if (task[i].type == "AllRoads")
            {
                if (!task[i].isDone())
                    task[i].TakeInfo(1);
            }
